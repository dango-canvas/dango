...
*   **设置面板 (`#settings-modal`)**:
    ...
    *   **画布底图**: 允许输入任意 HTTP/HTTPS 图片 URL，图片会以 `cover` 模式挂载在最底层 (`html` 标签)，并在上方 (`body` 标签) 覆盖一层无模糊的半透明灰色遮罩 (`rgba(127, 127, 127, 0.3)`)，完美保留网格线并确保节点易读性。该配置作为“画布氛围”的一部分，不仅持久化于本地，还会通过升级版协议 (Version 3) 随 JSON 文件和分享链接全量导出，同时保持对旧版本的向下兼容。
    *   手写风格 (实验性)。
